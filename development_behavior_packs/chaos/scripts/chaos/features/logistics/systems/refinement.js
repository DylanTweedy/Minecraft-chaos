...
export function createRefinementManager(cfg, deps) {
  const { FX, cacheManager, resolveBlockInfo, dropItemAt, fxManager, linkGraph } = deps;
  ...
  function sendExoticsToOutput(prismBlock, job, exotics) {
    try {
      ...
      for (const entry of exotics) {
        ...
        if (!scheduled) {
          const dropPos = outBlock?.location || prismBlock.location;
          dropItemAt(prismBlock.dimension, dropPos, entry.typeId, entry.amount);
        }
      }
    } catch (e) {
      // ignore
