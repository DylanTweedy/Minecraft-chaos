...
export function handleWandUseOn(e, deps) {
  try {
    ...
    // Check if it's any tier prism
    const isPrism = deps.isPrismBlock ? deps.isPrismBlock(block) : (typeId && typeId.startsWith("chaos:prism_"));
    const clickedType = isPrism ? "prism" : null;
